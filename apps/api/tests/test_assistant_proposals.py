...
class AssistantProposalTests(unittest.IsolatedAsyncioTestCase):
    def _request(self, mode: Literal["reconstruction", "search"] = "reconstruction") -> ProposalRequest:
    ...
    async def test_reconstruction_location_injection_is_blocked_and_discarded(self) -> None:
        ...
        case = case_from_dict(result.case)
        self.assertEqual(len(case.interaction_journal), 1)
        self.assertEqual(case.interaction_journal[0].id, "guard-proposal-1")
        self.assertEqual(case.interaction_journal[0].mode.value, "system")
        self.assertEqual(case.interaction_journal[0].text, "guard.ai_proposal_blocked")
        self.assertNotIn("машина", json.dumps(result.case, ensure_ascii=False))

    async def test_same_guard_request_is_idempotent_on_already_updated_case(self) -> None:
        fake = FakeProposalClient(
            ProposalModel(
                kind="next_action",
                target="машина",
        ...
        self.assertEqual(len(case_from_dict(second.case).interaction_journal), 1)

    async def test_probability_claim_is_blocked_in_search_mode(self) -> None:
        fake = FakeProposalClient(
            ProposalModel(
                kind="next_action",
