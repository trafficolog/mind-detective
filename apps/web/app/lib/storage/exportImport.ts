...
import { migrateAndValidateImportedCase } from './caseImport'

export function exportCase(caseValue: CaseV2): Blob {
  return new Blob([`${JSON.stringify(caseValue, null, 2)}\n`], { type: 'application/json' })
}

export async function importCase(file: File): Promise<CaseV2> {
  let payload: unknown
  try {
  ...
  return migrateAndValidateImportedCase(payload as Record<string, unknown>)
}
